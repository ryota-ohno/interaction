...
from make_8_xyz import exec_gjf
from vdw_8_xyz import vdw_R
    ...
    def get_init_para_csv(auto_dir,monomer_name):##初期値パラメータ決定
        ...
        for theta in tqdm(theta_list):
            ...
            for theta_ab in range(0,91):
                ...
                a=2*R_clps*np.cos(np.radians(theta_ab))##t-shaped基準でどれだけa方向にずらさないといけないのか *2に注意
                b=2*R_clps*np.sin(np.radians(theta_ab))##b
                if (a_clps > a) or (b_clps > b):##slipped parallelの方がずらさないといけない
                    continue##対称性を考えるとこの構造はリーズナブルではない
...
if __name__ == '__main__':
    ...
    if args.init:
        ...
        init_process(args)##まずinit　process
    
    print("----main process----")
    main_process(args)
    print("----finish process----")
##ある(a,b)についてその点を含んだ9点について計算を行い最小の点が端ならそれについて同様に9点の計算をする(終わったものは計算しない)これで安定構造を探索していくプロセス   
##クロスチェック　自分の今までの結果と比較　どこを改善できるか　ミスがないか　localやglobal　step間でのstackとか傾きの導入で挙動が変わってくる